...
with open('schemas/regions_schema.json', 'r') as f:
    schema = f.read()


def get_regions_df(app_name):
    ...
    df = spark \
        ...
        .option("subscribe", os.environ.get("TOPIC_REGIONS")) \
        .option("startingOffsets", "earliest") \
        .load()

    # Extract Values from cars_df
    df = df.selectExpr("substring(value, 6) as value") \
        .select(from_avro(col("value"), schema).alias("regions"))
    return df
