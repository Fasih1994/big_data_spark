...
with open('schemas/locations_schema.json', 'r') as f:
    schema = f.read()


def get_locations_df(app_name):
    ...
    df = spark \
        ...
        .option("subscribe", os.environ.get("TOPIC_LOCATIONS")) \
        .option("startingOffsets", "earliest") \
        .load()

    # Extract Values from cars_df
    df = df.selectExpr("substring(value, 6) as value") \
        .select(from_avro(col("value"), schema).alias("locations")) 
    return df
