...
with open('schemas/order_items_schema.json', 'r') as f:
    schema = f.read()


def get_order_items_df(app_name):
    ...
    df = spark \
        ...
        .option("subscribe", os.environ.get("TOPIC_ORDER_ITEMS")) \
        .option("startingOffsets", "earliest") \
        .load()

    # Extract Values from cars_df
    df = df.selectExpr("substring(value, 6) as value") \
        .select(from_avro(col("value"), schema).alias("order_items"))
    return df
